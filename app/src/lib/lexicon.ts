// Frontend-Port des Lexikon-Sentiments (supabase/functions/sentiment/lib/lexicon.ts).
// Gleiche Logik wie serverseitig — der News-Feed zeigt damit LIVE, wie jede
// Schlagzeile analysiert wurde (erkannte Ticker + Lexikon-Polaritaet).
...
export function analyzeText(text: string): TextSentiment {
  ...
  for (let i = 0; i < tokens.length; i++) {
    const base = BULLISH[tokens[i]] ?? BEARISH[tokens[i]];
  ...
  return { score: Math.max(-1, Math.min(1, sum / (hits * 1.5))), hits };
}

export function extractTickers(text: string, known: Set<string>): string[] {
  const out = new Set<string>();
  for (const m of text.matchAll(/\$([A-Za-z]{2,10})\b/g)) out.add(m[1].toUpperCase());
  ...
  return [...out].filter((s) => known.has(s));
}
